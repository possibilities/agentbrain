...
Text and local file bytes are captured as immutable Artifacts before acknowledgement. URL Admission performs syntax validation and queues normalized intent without network work. A new intent returns `queued`; a replay returns `duplicate` with the same job identity. A CLI URL submission whose conservative resource identity (x:status/x:article alias or normalized URL) already carries a materialized document returns `already_indexed` with the document id instead of queuing; `--force` queues rematerialization. The share ingress always queues. Materialization happens only after a Worker leases the job.

For URL jobs, the Worker invokes PATH-resolved `agentscrape fetch-markdown URL --envelope --allow-private-network --max-content-bytes N --max-relations N` without a shell; the durable admission of a URL carries the operator's browser-egress consent that Agentscrape requires for browser-backed live routes. Agentscrape owns fetching, browser/session behavior, redirects, credentials, backend retries, and extraction hardening. The live schema-v1 envelope must identify `agentscrape`; malformed and unknown versions are protocol defects. Agentbrain bounds and sanitizes extractor output and remains the only component allowed to mutate the index.
...
- `~/Library/LaunchAgents/agentbrain.doctor.plist` and a sibling `doctor.log`, running `agentbrain doctor --notify` on an interval. Admission acknowledges a submission long before extraction decides whether it succeeded, so a Stranded ingestion job has no request left to fail and nothing watching it; per [ADR 0018](docs/adr/0018-stranded-ingestion-is-reported.md) this is the reader. It is read-only and notifies only when the stranded count rises, so triage stays an explicit operator act;
