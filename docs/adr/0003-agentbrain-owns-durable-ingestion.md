...
The first research-index consolidation made Agentbrain the sole index writer while leaving saved-link admission in Linkctl and durable scrape work in Agentscrape. That boundary optimized for a compatibility cutover, but it cannot provide one inspectable lifecycle for every kind of ingestion. Synchronous text and file writes bypass durable intent, X child extraction is coupled to a root invocation, and future imports, recurring sources, and non-URL inputs need the same recovery and operator semantics.
...
- **Agentbrain workers delegate every URL extraction to Agentscrape.** Agentscrape remains the sole owner of network transport, browser/session behavior, URL security, redirects, provider-specific schemas, and extracted output. Agentbrain persists intent, invokes the bounded provider contract, validates its result, and owns retry scheduling and operator disposition.
- **Local and imported content use the same queue.** Text, files, directories, approved recovery artifacts, and later typed connectors enter through durable jobs even when no Agentscrape call is required.
...
- External extraction cannot run inside a database write transaction; workers must use short claims and fenced, idempotent completion around Agentscrape calls.
...
- Agentscrape's provider retry advice and extraction errors become inputs to Agentbrain's durable retry policy rather than an independent Agentbrain-targeted queue lifecycle.
- The Agentbot and Agentscrape cross-process contracts require versioned structured envelopes and coordinated rollout.
