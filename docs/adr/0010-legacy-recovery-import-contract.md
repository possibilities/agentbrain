...
- **Two human-authored Secretary links are approved for controlled online backfill.** The offline recovery run preserves them as non-runnable jobs. A separate online run is bound to the frozen generation digest, the offline run, and an immutable allowlist containing exactly those two opaque candidate evidence row IDs; neither rehearsal nor offline import fetches them.
...
- **Controlled execution is run-scoped.** Offline and online executors claim only jobs from their pinned run and allowed kinds while the ordinary worker is quiesced. The online run uses concurrency one; item-specific terminal failure does not suppress its sibling, while shared infrastructure, authorization, configuration, or integrity failure pauses the phase.
- **Online eligibility requires reconciled offline state, not fictional perfection.** The offline run must be terminal and exactly reconciled, with backup, integrity, artifact, and FTS gates passing. Isolated evidence-backed item failures may remain without authorizing unrelated work.
- **The online phase has its own rollback point.** A verified post-offline snapshot precedes egress. Successful sibling effects remain committed if one item fails; rollback concerns local DB/artifact effects and never claims to undo a remote request.
