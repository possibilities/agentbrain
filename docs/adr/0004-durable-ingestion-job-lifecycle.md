...
- **Infrastructure unavailability retries indefinitely.** Missing or unavailable Scrapectl/browser infrastructure uses capped exponential backoff with jitter and remains visible in `retry_wait` until service recovers or an operator intervenes.
...
- **External work occurs outside write transactions.** Workers claim briefly, perform Scrapectl or local materialization work without holding the writer lock, then commit through the fenced idempotent completion path.
...
This refines the durable-ingestion ownership decision in [ADR 0003](0003-agentbrain-owns-durable-ingestion.md) and retains the extraction boundary in [ADR 0002](0002-scrapectl-owns-url-extraction.md). See [`CONTEXT.md`](../../CONTEXT.md) for job, attempt, and run terminology.
