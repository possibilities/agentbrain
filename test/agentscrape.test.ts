...
test("an uncovered preset domain retries once with generic extraction", async () => {
  const url =
  ...
  const success = extractionEnvelope(url);
  // Refuse unless the caller forced generic extraction, exactly as the real
  // preset-owned domain check does.
  const { dir } = installAgentscrape(`#!/bin/sh
printf '%s\\n' "$@" >> "$LOG"
for arg in "$@"; do
  if [ "$arg" = "--generic" ]; then
    printf '%s' ${shellLiteral(JSON.stringify(success))}
    exit 0
  fi
done
printf '%s' ${shellLiteral(JSON.stringify(refusal))}
exit 1
`);
  process.env.LOG = join(dir, "generic-argv.txt");

  const result = await extractWithAgentscrape(url);

  expect(result).toMatchObject({ status: "success" });
  const argv = readFileSync(process.env.LOG, "utf8");
  // Two invocations: the preset attempt, then the generic retry.
  expect(argv.split("fetch-markdown").length - 1).toBe(2);
  expect(argv).toContain("--generic");
}, 30_000);

test("a permanent failure that is not a preset refusal does not retry", async () => {
...
exit 1
`);
  process.env.LOG = join(dir, "no-retry-argv.txt");

  await expect(extractWithAgentscrape(url)).rejects.toThrow(/invalid_request/);
  const argv = readFileSync(process.env.LOG, "utf8");
  expect(argv.split("fetch-markdown").length - 1).toBe(1);
  expect(argv).not.toContain("--generic");
}, 30_000);
